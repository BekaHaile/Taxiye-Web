...
const getSearch = async (key) => {
  ...
  if (q) {
    const res = await fetch(`${NEXT_PUBLIC_HOST}/articles/?featured=true&headerTitle_contains=${q}&_locale=${locale}`);
    return res.json();
  }
  const res = await fetch(`${NEXT_PUBLIC_HOST}/articles/?featured=true&_locale=${locale}`);
  return res.json();
}
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const articleRes = await fetch(`${NEXT_PUBLIC_HOST}/articles/?article_id=${id}&_locale=${locale}`);
    const articlesData = await articleRes.json();
    const popularRes = await fetch(`${NEXT_PUBLIC_HOST}/articles/?featured=true&_locale=${locale}`);
...
export default function articles({ articlePage, article, populars, error }) {
  ...
  const { data, status } = useQuery(['articles', { content: searchText, locale:router?.locale }], getSearch, { initialData: populars });
