...
export const corporate = (store) => (next) => async (action) => {
    ...
    else if (action.type == "ADMIN_NAME_ADDED" ||
        action.type == "ADMIN_EMAIL_ADDED" ||
        action.type == "ADMIN_PASSWORD_ADDED" ||
        action.type == "ADMIN_CONFIRMATION_PASSWORD_ADDED") {
  
        if (data["admin_full_name"] != "" && data["admin_full_name"] != null &&
            data["admin_email"] != "" && data["admin_email"] != null &&
            data["password"] != "" && data["password"] != null &&
            data["confirmation_password"] != "" && data["confirmation_password"] != null &&
            data["password"] == data["confirmation_password"] && validateEmail(data["admin_email"]) &&
            data["password"].length >= 6
        )
            next(actions.setAdminValidation(true));
        else
            next(actions.setAdminValidation(false));
